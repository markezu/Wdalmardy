...
export default function Sidebar({ user }: { user: AdminUser }) {
  ...
  const items: NavItem[] = useMemo(
    () => [
      ...
      { href: '/admin/offers', label: 'العروض والخصومات', icon: Megaphone, comingSoon: true },
      { href: '/admin/delivery', label: 'إدارة التوصيل', icon: Truck, comingSoon: true },
      { href: '/admin/reports', label: 'التقارير', icon: BarChart3, permission: 'reports.view', comingSoon: true },
      { href: '/admin/permissions', label: 'صلاحيات المدير', icon: ShieldCheck, permission: 'users.manage', comingSoon: true },
